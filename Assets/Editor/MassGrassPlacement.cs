...
public class MassGrassPlacement : EditorWindow
{
    ...
    public float channelIntersection = 0.8f;
    [MenuItem("Tools/Terrain/Auto Grass Placement")]
    static void Init()
    {
        MassGrassPlacement window = (MassGrassPlacement)GetWindow(typeof(MassGrassPlacement));
        window.Show();
    ...
    void OnGUI()
    {
        ...
        if (terrain != null)
        {
            ...
            channelIntersection = EditorGUILayout.Slider(new GUIContent("Intersection: ", "How much grass on channels intersections"), channelIntersection, 0, 1);
        ...
        for (int i = 0; i < splatTextureIndicesToAffect.Length; i++)
        {
            for (int j = 0; j < detailWidth; j++)
            {
                for (int k = 0; k < detailHeight; k++)
                {
                    ...
                    if (alphaValue < channelIntersection)
                    {
                        continue;
                    }
